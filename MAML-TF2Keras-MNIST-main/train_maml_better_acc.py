...
opt_outter = tfa.optimizers.NovoGrad(outter_lr, clipnorm=1.0)
...
def cnn():
    ...
    fc1 = tf.keras.layers.Dense(
        128, kernel_initializer="he_uniform", activation=tf.nn.relu
    )(fc)
    fc2 = tf.keras.layers.Dense(
        256, kernel_initializer="he_uniform", activation=tf.nn.relu
    )(fc1)
    fc3 = tf.keras.layers.Dense(
        512, kernel_initializer="he_uniform", activation=tf.nn.relu
    )(fc2)
    out = tf.keras.layers.Dense(1, kernel_initializer="he_uniform", activation=None)(
        fc3
    )
...
kmnist_tr_ns = kmnist_tr.apply(
    lambda ds: ds.map(lambda x, y: (tf.reshape(x, [1, 28, 28, 1]), y))
)
kmnist_ts_ns = kmnist_ts.apply(
    lambda ds: ds.map(lambda x, y: (tf.reshape(x, [1, 28, 28, 1]), y))
)
...
for ofd in out_of_distro_classes_list:
    ...
    for ofd_inner in out_of_distro_classes_list:
        ...
        if is_first_time:
            ...
            in_of_distro_tr[ofd] = in_of_distro_tr[ofd].concatenate(
                out_of_distro_tr[ofd_inner]
            )
...
_ = cnn_model(
    support_set
)  # 1)for initializing the model. 2)for define the loss function out-of the loop
loss_inner_meta = lambda: tf.reduce_mean(
    tf.nn.sigmoid_cross_entropy_with_logits(
        labels=meta_labs, logits=cnn_model(support_set)
    )
)
loss_inner_task = lambda: tf.reduce_mean(
    tf.nn.sigmoid_cross_entropy_with_logits(
        labels=meta_labs, logits=cnn_model(query_set)
    )
)
...
def eval_model(mode_binary_classifier):
    ...
    if mode_binary_classifier == "OneVsRest":
        ...
        for ofd_outer in out_of_distro_classes_list:
            for ofd_inner in out_of_distro_classes_list:
                if (
                    (ofd_outer, ofd_inner) in used_pairs
                    or (ofd_inner, ofd_outer) in used_pairs
                    or ofd_inner == ofd_outer
                ):
                    break
                ...
                cloned_models[(ofd_outer, ofd_inner)] = tf.keras.models.clone_model(
                    cnn_model
                )
...
summary_writer = SummaryWriter(
    log_dir=f"run/{time.time()}_{n_first_training=}_{n_second_training=}"
)
for step in range(50000):
    # keep the original weights for meta-training
    meta_weights = [
        tf.Variable(target_weights) for target_weights in cnn_model.trainable_weights
    ]
    ...
    def meta_loss():
        ...
        for task_count in range(inner_task_loop_no):
            ...
            for model_weight_index in range(len(cnn_model.trainable_weights)):
                cnn_model.trainable_weights[model_weight_index].assign(
                    meta_weights[model_weight_index]
                )
            pass

            # updateing the weights from meta-weights
            opt_inner.minimize(
                loss=loss_inner_meta, var_list=cnn_model.trainable_weights
            )
