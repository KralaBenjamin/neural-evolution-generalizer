...
from torchvision.datasets import CIFAR10, CIFAR100, KMNIST, MNIST, FashionMNIST
...
father_weights = conv_init(
    rng, (batch_size, data_image_size, data_image_size, data_image_depth)
)
...
x_train = x_test[
    random.randint(rng, (n_offsp_epoch * n_samples,), 0, 15000, dtype="uint8")
]
...
imgs = conv_apply(
    father_weights,
    testaffe.reshape(-1, data_image_size, data_image_size, data_image_depth),
)
...
def accuracy(conv_weights, MLP_params, dataset_imgs, dataset_lbls):
    ...
    imgs = conv_apply(
        conv_weights,
        dataset_imgs.reshape(-1, data_image_size, data_image_size, data_image_depth),
    )
    pred_classes = jnp.argmax(
        jit_batched_MLP_predict(MLP_params, imgs.reshape(-1, NNin1)), axis=1
    )
...
def train(conv_weights, imgs, lbls, MLP_params):
    for n in range(n_training_epochs):
        for i in range(jnp.shape(imgs)[0]):
            ...
            img_conv = conv_apply(
                conv_weights,
                imgs[i].reshape(-1, data_image_size, data_image_size, data_image_depth),
            )
...
best_weights = conv_init(
    father_key, (batch_size, data_image_size, data_image_size, data_image_depth)
)[1]
common_start_acc = 0
training_cat_means = []
testing_cat_means = []
...
for meta in range(n_metaepochs):
    ...
    if meta == 0:
        ...
        if use_all_pickle:
            ...
            father_weights = conv_init(
                father_key,
                (batch_size, data_image_size, data_image_size, data_image_depth),
            )
    ...
    x_test = x_test.reshape(
        n_offsp_epoch, n_test, data_image_size, data_image_size, data_image_depth
    )
